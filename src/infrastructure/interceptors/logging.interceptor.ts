...
import { setLoggerContext } from 'src/modules/logger/logger-context';
...
export class LoggingInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    setLoggerContext({ uuid, controllerName, methodName });

    const url = request.originalUrl || request.url;

    this.logger.log(`[${uuid}] Executing method.`, method);
    this.logger.log(`[${uuid}] URL: ${url}`, method); 
    const now = Date.now();

    const requestData = JSON.stringify(request.body) || '';

    this.logger.log(`[${uuid}] Request body: ${requestData}`, method);

    return next.handle().pipe(
      tap((data) => {
        if (data) {
          const responseData = JSON.stringify(data) || '';
          this.logger.log(`[${uuid}] Response body: ${responseData}`, method); 
        }
